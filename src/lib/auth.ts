...
export async function signAuthToken(payload: AuthTokenPayload) {
  try {
    ...
    logToSentry("Token signing failed", "auth", { payload }, "error", error);
...
export async function setAuthCookie(token: string) {
  try {
    ...
    logToSentry("Failed to set cookie", "auth", { token }, "error", error);
  }
}
...
export async function removeAuthCookie() {
  try {
    ...
    logToSentry("Failed to remove the auth cookie", "auth", {}, "error", error);
  }
}
